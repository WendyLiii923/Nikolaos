		...
		<div class="h-100 p-3 border-right" style="width: 200px">
			<div class="list-group list-group-flush">
				<c:forEach var="category" items="${categoryList}">
					<tr>
						<td><a
							class="list-group-item list-group-item-action <c:if test='${param.categoryId == category.id}'>active</c:if>"
			...
			<div
				...
				<div class="d-flex align-items-center">
					...
					<form class="d-flex" action="<c:url value='/used' />" method="GET">
						...
						<input type="hidden" name='status' value='價格區間'> <input
							type='text' style="width: 80px;" name='start' value='${start}'
							placeholder="最低價"> <span style="line-height: 35px">~</span>
						<input type='text' style="width: 80px;" name='end' value='${end}'
							placeholder="最高價"> <input type="submit" class="d-none"
							value="確定">
						<P />
					</form>
				</div>
				...
				<div>
					<c:set var="q" value="${pageContext.request.queryString}"
						scope="page" />
					<a class="text-white"
						href='<c:url value="/used?${q.contains('&type') ? q.substring(0,q.indexOf('&type')) : q}&type=card" />'>
						<i class="fa fa-2x fa-th-large mr-3 " aria-hidden="true"></i>
					</a> <a class="text-white"
						href='<c:url value="/used?${q.contains('&type') ? q.substring(0,q.indexOf('&type')) : q}&type=list" />'>
			...
			<div class="d-flex">
				<div class="p-3 w-100">
					<c:if test="${empty param.type or param.type == 'list'}">
						<c:forEach var="product" items="${productList}">
							<div class="d-flex shadow rounded p-2 mb-3"
								...
								<div class="pl-3"
									style="height: 150px; width: calc(100% - 300px);">
					...
					<c:if test="${param.type == 'card'}">
						...
						<c:forEach begin="0" step="4" end="${4 * rowCount}"
